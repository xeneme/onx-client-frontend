<template>
  <div class="withdrawals-table" v-if="value && value.length && !loading">
    <div
      class="withdrawals-table__items"
      :style="{
        height: range * 40 + 30 + 'px',
      }"
    >
      <div class="withdrawals-table__items__title">
        <div class="withdrawals-table__items__title-amount">
          Date
        </div>
        <div class="withdrawals-table__items__title-amount">
          Amount
        </div>
        <div class="withdrawals-table__items__title-status">
          Status
        </div>
      </div>
      <div
        class="withdrawals-table__items__item"
        v-for="item in listPageItems"
        :key="item._id"
      >
        <div class="withdrawals-table__items__item-date">
          {{ formatDate(item.at) }}
        </div>
        <div class="withdrawals-table__items__item-amount">
          {{ formatAmount(item) }}
        </div>
  ...
  <NoData
    ...
    LT="Your withdrawals are loading..."
...
export default {
  ...
  methods: {
    ...
    goto(page) {
      ...
      this.$anime({
        targets: '.withdrawals-table__items__item',
        opacity: [1, 0],
        duration: 200,
        easing: 'easeInSine',
        complete() {
